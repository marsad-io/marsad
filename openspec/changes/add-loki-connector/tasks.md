...
- [x] 3.1 Test (httptest fake Loki): `search_logs` maps selector/time range/limit/direction to `/loki/api/v1/query_range` and parses streams into the neutral result shape -> implement `internal/connector/loki`
...
- [x] 3.4 Test: bearer/basic auth via env references only; inline secrets rejected by config -> implement auth plumbing
- [x] 3.5 Integration test (testcontainers, real Loki image): seed logs, search end to end
...
- [x] 4.2 E2e smoke: MCP client -> stdio server -> Prometheus + Loki containers -> metric query and log search in one session; audit verified
